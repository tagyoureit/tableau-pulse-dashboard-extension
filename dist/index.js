...
const crypto_js_1 = __importDefault(require("crypto-js")); // Importing CryptoJS properly
const path = require("path");
const https_1 = __importDefault(require("https"));
const fs_1 = __importDefault(require("fs"));
const Interface_1 = require("./extension/Interface");
const uuid = require('uuid');
const TokenManager_1 = require("./extension/TokenManager");
...
// Create HTTPS server
const privateKey = fs_1.default.readFileSync(path.resolve('./sslcert/server.key'), 'utf8'); // Path to your private key
const certificate = fs_1.default.readFileSync(path.resolve('./sslcert/server.crt'), 'utf8'); // Path to your SSL certificate
...
async function apiAuth() {
    ...
    // Get token
    const cachedToken = TokenManager_1.tokenManager.getToken(settings.caClientId, settings.caSecretId);
    ...
    let apiResponse = await (0, axios_1.default)({
        method: "post",
        url: apiUrl,
        headers: {
    ...
    TokenManager_1.tokenManager.addToken(settings.caClientId, settings.caSecretId, userId, myToken.exp, token);
    return [token, userId];
}
app.put("/data", async (request, response) => {
    try {
        const settingsBody = request.body;
...
function getJWT() {
    ...
    const stringifiedHeader = crypto_js_1.default.enc.Utf8.parse(JSON.stringify(header));
    const encodedHeader = base64url(stringifiedHeader);
    const stringifiedPayload = crypto_js_1.default.enc.Utf8.parse(JSON.stringify(payload));
    const encodedPayload = base64url(stringifiedPayload);
    // Create Signature
    const signature = crypto_js_1.default.HmacSHA256(encodedHeader + "." + encodedPayload, settings.caSecretValue);
...
async function apiSubscribedMetrics(token, userId) {
    ...
    const myApi = await (0, axios_1.default)({
        method: "get",
...
async function apiMetricsArray(token, userSubscribedMetrics) {
    ...
    for (const n of userSubscribedMetrics) {
        const getMetric = await (0, axios_1.default)({
            method: "get",
...
async function apiDefinitionsArray(token, definitionIds) {
    ...
    for (const i of definitionIds) {
        ...
        const getDefinitions = await (0, axios_1.default)({
            method: "get",
    ...
    for (let i = 0; i < Math.min(metricsArray.length, definitionsJsonObject.length); i++) {
        const combinedObject = {
            "bundle_request": {
                "version": 1,
                "options": {
                    "output_format": Interface_1.OutputFormat.TEXT,
