...
(function () {
  ...
  $(function () {
    ...
    tableau.extensions.initializeDialogAsync().then(function (settingsStr) {
      ...
      let settingsDefault = { ...JSON.parse(settingsStr) as ISettings };
      settings = {
  ...
  function attachEventListeners() {
    ...
    $('#tableauUrl').on('change', function () {
      ...
      settings.tableauUrl = `https://${$(this).val() as string}.online.tableau.com`;
      console.log(JSON.stringify(settings, null, 2));
    });

    // Set Site Name
    $('#siteName').on('change', function () {
      settings.siteName = $(this).val() as string;
    });

    // Set User Name
    $('#userName').on('change', function () {
      settings.userName = $(this).val() as string;
    });

    // Set Client ID
    $('#caClientId').on('change', function () {
      settings.caClientId = $(this).val() as string;
    });

    // Set Secret ID
    $('#caSecretId').on('change', function () {
      settings.caSecretId = $(this).val() as string;
    });

    // Set Secret Value
    $('#caSecretValue').on('change', function () {
      settings.caSecretValue = $(this).val() as string;
    });
  }
